...
<html>
	...
	<head>
		...
		<style>
		/* Yasitha: Original code. Commented to hide the table and only show the snap IDE.
/* 			html { */
/* 				height: 100%; */
/* 			} */
/* 			body { */
...
/* 			} */
            ...
            /* Hide the compact arrow button completely */
            #compact {
                display: none !important;
            }

            /* Sidebar (Snap IDE) takes full width */
            #sidebar {
                width: 100%;
                height: 100%;
                float: none;
            ...
            /* Hide the right-hand content panel entirely */
            #content {
                display: none;
            }
		...
		<script type="text/javascript">
			function loadSnap(id, project, currSpriteName) {
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState==4 && xhr.status==200) {
                        ...
                        xml = xml.replace(/^\uFEFF/, '');   // strip BOM if present
                        contentWindow.ide.droppedText(xml, 'project.xml', 'text/xml');
			...
			function selectSprite(contentWindow, currSpriteName) {
              try {
                var ide = contentWindow.ide;
                if (!ide || !ide.stage) return;

                // normalize sprite name
                var name = (currSpriteName || '').toString();
                name = name.replace(/^"+|"+$/g, '');

                // stage + sprites
                var candidates = [ide.stage].concat(ide.stage.children || []);

                var sprite = candidates.find(function (s) {
                  return s && typeof s.name === 'string' && s.name === name;
                });

                if (sprite && typeof ide.selectSprite === 'function') {
                  ide.selectSprite(sprite);
                }
              } catch (e) {
                console.warn('selectSprite failed:', e);
              }
            }
...
if ($enable_viewer) {
	...
	$query = "SELECT id, serverTime, message, data, code <> '' AS link, sessionID FROM $table $where ORDER BY serverTime, id";
	echo $query;
	...
	while($row = mysqli_fetch_array($result)) {
		...
		$class = "";
		if ($link) {
			$jsSprite = json_encode($current_sprite_name); // safe JS string
            $first = "<a href='#$rid' onclick='loadSnap(\"$rid\", \"$id\", $jsSprite)'>$first</a>";
			$class = "rid";
		}
