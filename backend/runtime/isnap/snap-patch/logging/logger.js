...
Logger.prototype.simpleCodeXML = function () {
    ...
    try {
        // Workaround for Cellular since it uses a different serializer
        var xml = null;
        var environment = localStorage.getItem('assignmentEnvironment');
        if (environment.toLowerCase() == 'cellular') {
            var xml = this.serializer.serialize(ide.stage);
        } else {
            var xml = ide.getProjectXML();
        }
    } finally {
...
extend(IDE_Morph, 'openProject', function (base, project, purgeCustomizedPrims) {
    Logger.openingProject = true;
    base.call(this, project, purgeCustomizedPrims);
