...
extend(SnapSerializer, 'load', function (base, xmlString, ide, noPrims) {
    // call Snap's real loader
    var result = base.call(this, xmlString, ide, noPrims);

    // keep iSnap's assignment behavior
    Assignment.updateStageAssignment();

    // When a project is opened, update the current assignment to match
    if (ide && ide.stage && ide.stage.assignment !== Assignment.getID() &&
        window.assignments) {

        var currentAssignment = Assignment.get();
        var projectAssignmentID = ide.stage.assignment;

        // Make sure the project's assignment is listed and not a prequel
        if (window.assignments[projectAssignmentID] &&
            (!currentAssignment || currentAssignment.prequel !== projectAssignmentID)) {
